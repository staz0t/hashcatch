...
hc_setup(){
	if [ "$EUID" == 0 ]
	then
		echo "[-] Do not run setup with root privileges. Password will be requested when required!"
		exit 0
	fi
	banner
...
hc_run(){
	...
	then
		echo -e "[-] Essential file(s) missing. Run setup and try again!"
		exit 0
	elif [ ! `grep -i "interface" config | awk -F'=' '{print $2}'` ]
	then
		echo -e "[-] Interface not mentioned in config file. Run setup and try again!"
		exit 0
	fi
